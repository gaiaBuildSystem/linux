...
static int tegra_i2c_probe(struct platform_device *pdev)
{
	 ...
	 * be used for atomic transfers. ACPI device and RT PM case are not IRQ safe also.
	 */
	if (!IS_VI(i2c_dev) && !has_acpi_companion(i2c_dev->dev) && !IS_ENABLED(CONFIG_PREEMPT_RT))
