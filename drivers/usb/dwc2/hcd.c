				       ...
				       struct dwc2_host_chan *chan, u32 *hcchar)
{
	if (chan->ep_type == USB_ENDPOINT_XFER_INT ||
		...
		/*
		 * Confirmed with synopsys, period packet must be send out in the next frame.
		 * So if the current frame is odd, clear the odd flag; or else, set the odd
		 * flag. Previous logic has bugs in some corner cases, and this logic is fine
		 * and simple.
		 */
		if (hfnum & 1)
